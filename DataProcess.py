...
class DataProcess(InMemoryDataset):
    def __init__(self, root, pair, rate=0.3, seed=1, transform=None, pre_transform=None):
        self.pair = pair
        # self.KG_num = KG_num
        self.rate = rate
        self.seed = seed
    ...
    def processed_file_names(self):  # 存放处理后的数据，一般是pt格式
        return '%s_%.2f_%d.pt' % (self.pair, self.rate, self.seed)
    ...
    def process(self):
        ...
        #     data = Data(x=x, edge_index=edge_index, rel=rel, pair_set=pair_set1, train_set=train_set.t(), test_set=test_set.t())
    ...
    def process_graph(self, triple_path, ent_path, emb_path):
        ...
        embedding_list=torch.rand(-1, 1, [max(idx),300])
