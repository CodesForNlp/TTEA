...
class GAT_E_to_R(nn.Module):
    ...
    def forward(self, x_e, edge_index, rel, rel_size):
        ...
        x_triple = self.s_r2(torch.cat([x_e[edge_index_h], x_res2, x_e[edge_index_t]], dim=1))
        x_r_h = self.w_h(x_e[edge_index_h])
        x_r_t = self.w_t(x_e[edge_index_t])
        e1 = self.a_h1(x_r_h).squeeze() + self.a_t1(x_triple).squeeze()
        e2 = self.a_t2(x_triple).squeeze() + self.a_h2(x_r_t).squeeze()
        e3 = self.a_t3(x_triple).squeeze() + self.a_h3(x_res2+x_res).squeeze()
...
class GATt_to_R(nn.Module):
    ...
    def forward(self,x_e,edge_index,rel,x_res1, rel_size):
        edge_index_h, edge_index_t = edge_index
        s_convert_t = torch.tanh(self.t_c1(x_e))
        ...
        x_triple=self.s_r2(torch.cat([s_convert_t[0], x_res2+x_res, s_convert_t[1]], dim=1))
        e1 = self.a_1(x_res1).squeeze() + self.a_2(x_triple).squeeze()
        ...
        x_r_r1 = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max() + 1, edge_index_t.size(0),
                      x_triple)
        e2 = self.a_3(x_res1).squeeze() + self.a_4(x_triple).squeeze()
        alpha = softmax(F.leaky_relu(e2).float(), rel)
        x_r_t1 = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max() + 1, edge_index_t.size(0),
                      x_res1)
        x_type = x_r_r1+x_r_t1
        x_res1 = x_type[rel]+x_res1+x_triple
